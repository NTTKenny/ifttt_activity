    ...
    for i, integer in enumerate(index_list):
        ...
        if integer == index_list[-1]:
            if integer - 1 == index_list[-2]:
                return_list.append((integer, index_list[-1]))
                break
...
# # TODO ACTIVITY_LIST IS NOT CREATED YET.
activity_list = [Cycle.mountain_biking]
    ...
    for activity_requirement_values in activity_list:
        ...
        if ocean_parameter_keys != None:
            for (start, end) in suitable_windows_index:
                oww = [value for value in list(beach_forecast.keys()) if forecast_keys[start] <= value <= forecast_keys[end]]
                for time in oww:
...
land_activity_times = (times_for_activity(activity_list=activity_list, land_parameter_keys=land_keys))
