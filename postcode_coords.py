...
class PostCode:
    ...
    def postcode_to_coord(self, post_code):
        post_code = 'bh13 7jh'
        url = "https://api.postcodes.io/postcodes/{postcode}".format(postcode=post_code)
        response = requests.get(url).json()['result']
        
        postcode_coords = {'lon':response['longitude'], 'lat': response['latitude']}
    ...
    def closest_beach(self, user_coordinate_dict, surf_locations: list):
        ...
        for location_dict in surf_locations:
           ...
           distance = root((user_lon - surf_lon)**2 + (user_lat - surf_lat)**2)
